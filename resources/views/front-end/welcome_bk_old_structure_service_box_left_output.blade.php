...
<div class="as-hero-wrapper hero-3 {{ !empty($serviceName != 'Institute of Professional Training & Management (IPTM)' ) ? 'slider-reverse-main' : '' }}">
                    ...
                    <div class="tab-content" id="myTabContent">
						...
						<div class="tab-pane fade show active" id="" >
							...
							<div class="title-area mb-25">
                                <h3 class="sec-title fw-semibold" id="title-practice-area">Capacity Building & k.a.s.e Development Program (Our
                                    Achievement)
                                </h3>
                            </div>
                            <p class="mt-n2 mb-25" id="description-practice-area">Lorem ipsum is a placeholder text commonly used to demonstrate the
                                visual
                                form of
                                a document or a typeface without relying on meaningful content.
							</p>
                            <div class="checklist mb-30">
                                <ul id="results-practice-area"></ul>
							...
							@if (!empty($services))
								...
								@foreach ($services as $service)
									...
									<li class="nav-item" role="presentation">
										<button class="nav-link" id="box{{ $service['id'] }}" onclick="practiceServiceTab({{ $service['id'] }},0,1)">
							...
							<div class="custom-tabs single-tabs ">
								<ul class="nav nav-tabs" id="myTab" role="tablist">
									...
									@foreach( $serviceCoverArea  as $serAreaName)	
										...
										<li class="nav-item" role="presentation">
											<button class="nav-link active"  id="box{{ !empty($serAreaName['service_id']) ? $serAreaName['service_id'] : '' }}" onclick="practiceServiceTab({{ !empty($serAreaName['service_id']) ? $serAreaName['service_id'] : 0 }},{{ !empty($serAreaName['id']) ? $serAreaName['id'] : 0 }},1)">
												<div class="feature-card-wrap">
													<div class="feature-card">
														...
														<h3 class="feature-card_title">{{ !empty($serviceCoverAreaList[$serAreaName['id']]['area_name']) ? $serviceCoverAreaList[$serAreaName['id']]['area_name'] : '' }}</h3>
													</div>
												</div>
											</button>
										</li>
						...
						@elseif( $totalServiceCoverArea == 2 )
							...
							<div class="custom-tabs four-tabs ">
								<ul class="nav nav-tabs" id="myTab" role="tablist">
									...
									<li class="nav-item" role="presentation">
										<button  id="box{{$serAreaName['id']}}" class="nav-link" onclick="practiceServiceTab({{ !empty($serAreaName['service_id']) ? $serAreaName['service_id'] : 0 }},{{ !empty($serAreaName['id']) ? $serAreaName['id'] : 0 }},1)" >
											<div class="feature-card-wrap">
												<div class="feature-card">
													<div class="feature-card_icon"><img
															src="{{ asset('storage/app/public/service-cover-areas/'.$areaIcon) }}"
							...
							{{-- this is for 3 tabs start from here   --}}
								<div class="custom-tabs three-tabs ">
									...
									@foreach( $serviceCoverArea  as $serAreaName)
										...
										<li class="nav-item" role="presentation">
											<button class="nav-link active"  id="box{{ !empty($serAreaName['service_id']) ? $serAreaName['service_id'] : 0 }}" onclick="practiceServiceTab({{ !empty($serAreaName['service_id']) ? $serAreaName['service_id'] : 0 }},{{ !empty($serAreaName['id']) ? $serAreaName['id'] : 0 }},1)">
								...
								<div class="custom-tabs four-tabs ">
									<ul class="nav nav-tabs" id="myTab" role="tablist">
										...
										@foreach( $serviceCoverArea  as $serAreaName)	
												...
												<li class="nav-item" role="presentation">
													<button class="nav-link active"  id="box{{ !empty($serAreaName['service_id']) ? $serAreaName['service_id'] : 0 }}" onclick="practiceServiceTab({{ !empty($serAreaName['service_id']) ? $serAreaName['service_id'] : 0 }},{{ !empty($serAreaName['service_cover_area_id']) ? $serAreaName['service_cover_area_id'] : 0 }},1)">
							...
							<ul class="nav nav-tabs" id="myTab" role="tablist">
								...
								@foreach( $serviceCoverArea  as $serAreaName)	
									...
									@php $areaIcon = !empty($serviceCoverAreaList[$serAreaName['id']]['area_icon']) ? $serviceCoverAreaList[$serAreaName['id']]['area_icon'] : ''; @endphp
										...
										<li class="nav-item" role="presentation">
										<button class="nav-link "  id="box{{$serAreaName['service_id']}}" onclick="practiceServiceTab({{ !empty($serAreaName['service_id']) ? $serAreaName['service_id'] : 0 }},{{ !empty($serAreaName['id']) ? $serAreaName['id'] : 0 }},1)">
						...
						@elseif( $totalServiceCoverArea == 6 )
							...
							<div class="custom-tabs six-tabs ">
								<ul class="nav nav-tabs" id="myTab" role="tablist">
									...
									<li class="nav-item" role="presentation">
										<button class="nav-link active" id="box{{$serAreaName['service_id']}}" onclick="practiceServiceTab({{ !empty($serAreaName['service_id']) ? $serAreaName['service_id'] : 0 }},{{ !empty($serAreaName['id']) ? $serAreaName['id'] : 0 }},1)">
											<div class="feature-card-wrap">
												<div class="feature-card">
													<div class="feature-card_icon"><img
    ...
    <script>
		...
		// start practice/service tab 
			//var serviceId = '<?=$serviceIDForPracticeLeftContent?>';
			var serviceId = 3;
			var serviceAreaId = '<?=$serviceAreaIdForPracticeLeftConten?>';;
			var pagePractice = 1;
			practiceServiceTab(serviceId,serviceAreaId,pagePractice);
			
			$("#prv-practice-area").click(function() {
				$("#results-practice-areat").html('');
				pagePractice--;
				//alert(pagePractice);
				practiceServiceTab(serviceId,serviceAreaId,pagePractice);
			});
			...
			function practiceServiceTab(serviceId,serviceAreaId,pagePractice)
			{
				//alert(serviceId);
				...
				$.ajax({
                    url: site_url + "/portal-practice-area/"+serviceId+"/"+serviceAreaId+"?page=" + pagePractice,
                    type: "get",
                    datatype: "html",
                    beforeSend: function() {
                ...
                .done(function(data) {
					...
					if (data.total_count < 17) {
						
                        $('.prv-practice-area').css('display', 'none');
                        $('.lod-practice-area').css('display', 'none');
                    }
					
                    var maxPage = data.total_count / 16;
